...
function ensureBenchmarkUi(): void {
  ...
  runBtn.addEventListener("click", () => {
    const games = Math.max(1, Math.min(500, Math.floor(Number(gamesInput.value) || 8))); const maxPieces = Math.max(20, Math.min(2000, Math.floor(Number(maxPiecesInput.value) || 250))); const seedBase = Math.floor(Number(seedInput.value) || 1337);
    ...
    worker.postMessage({ type: "run", config: { games, maxPieces, seedBase } });
  });
}
ensureBenchmarkUi();
